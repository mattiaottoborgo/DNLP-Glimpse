...
def summarize(text1, text2, text3, iterations, rationality=1.0):
    ...
    ##tuple obtained by rsa_reranker.rerank
    (
        best_rsa,
        best_base,
        speaker_df,
    ...
    # apply exp to the probabilities --> these are the uniqueness scores.
    speaker_df = speaker_df.applymap(lambda x: math.exp(x))

    text_1_summaries = speaker_df.loc[text1][text1_sentences]  #TODO: what is this? check id in demo
    ...
    latex = latex.replace("[REVIEW 3]", latex_text_3)
             ...
             #uniqueneness,   uniqueness,      uniqueness
...
# call the summarize . Get uniqueness scores for each sentence in each text from text_1_summaries,...
iface = gr.Interface(
    fn=summarize,
    inputs=[
